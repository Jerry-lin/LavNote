...
namespace bs {
    ...
    void dfs_out(int id,node &u) {
        //u.vis = vis_cur;
        nodeLabel3[nodeLabel1[id].oldID_mapto_newID].vis=vis_cur;
        nodeLabel3[nodeLabel1[id].oldID_mapto_newID].L_interval.first = cur++;
        
        if (u.N_O_SZ==0) {
            nodeLabel3[nodeLabel1[id].oldID_mapto_newID].h_out = h_out() % (K * 32);
        } else {
            for (int i = 0; i < K; i++) {
                nodeLabel3[nodeLabel1[id].oldID_mapto_newID].L_out[i] = 0;
            }
            
            for (int i = 0; i < u.N_O_SZ; i++) {
                int id2=u.N_O[i];
                node &v = nodes[u.N_O[i]];
                if (nodeLabel3[nodeLabel1[id2].oldID_mapto_newID].vis != vis_cur) {
                    dfs_out(id2,v);
                }
                if (v.N_O_SZ==0) {
                    int hu = nodeLabel3[nodeLabel1[id2].oldID_mapto_newID].h_out;
                    nodeLabel3[nodeLabel1[id].oldID_mapto_newID].L_out[(hu >> 5) % K] |= 1 << (hu & 31);
                } else {
                    for (int j = 0; j < K; j++) {
                        nodeLabel3[nodeLabel1[id].oldID_mapto_newID].L_out[j] |= nodeLabel3[nodeLabel1[id2].oldID_mapto_newID].L_out[j];
                    }
                }
            }
        ...
        nodeLabel3[nodeLabel1[id].oldID_mapto_newID].L_interval.second = cur;
    }
    ...
    void calculate()
    {
        vis_cur++;
        cur=0;
        for (int i=0; i<temp_root_out.size(); i++) {
            dfs_out(temp_root_out[i],nodes[temp_root_out[i]]);
        }
    }
    ...
    void run_queries() {
        ...
        for(vector<int>::iterator x=src.begin(), y=dest.begin(); x!=src.end(); x++, y++) {
            ...
            if(*x == *y)
                ...
                if (nodeLabel3[nodeLabel1[*x].oldID_mapto_newID].L_interval.second < nodeLabel3[nodeLabel1[*y].oldID_mapto_newID].L_interval.second) {
                    r= false;
                } else if (nodeLabel3[nodeLabel1[*x].oldID_mapto_newID].L_interval.first <= nodeLabel3[nodeLabel1[*y].oldID_mapto_newID].L_interval.first) {
                    t=true;
                }
                if(!t&&r)
                {
                    if (nodes[*y].N_O_SZ==0) {
                        if ((nodeLabel3[nodeLabel1[*x].oldID_mapto_newID].L_out[nodeLabel3[nodeLabel1[*y].oldID_mapto_newID].h_out >> 5] & (1 << (nodeLabel3[nodeLabel1[*y].oldID_mapto_newID].h_out & 31))) == 0) {
                            r= false;
                        }
                    } else {
                        for (int i = 0; i < K; i++) {
                            if ((nodeLabel3[nodeLabel1[*x].oldID_mapto_newID].L_out[i] & nodeLabel3[nodeLabel1[*y].oldID_mapto_newID].L_out[i]) != nodeLabel3[nodeLabel1[*y].oldID_mapto_newID].L_out[i]) {
                                r=false;
                            }
                        }
                    }
