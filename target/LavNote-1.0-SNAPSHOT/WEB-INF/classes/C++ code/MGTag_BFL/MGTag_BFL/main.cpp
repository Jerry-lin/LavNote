...
namespace bs {
    ...
    void dfs_out(int id,node &u) {
        ...
        if (u.N_O_SZ==0) {
            nodeLabel2[nodeLabel1[id].oldID_mapto_newID].h_out = h_out() % (K * 32);
        } else {
            for (int i = 0; i < K; i++) {
                nodeLabel2[nodeLabel1[id].oldID_mapto_newID].L_out[i] = 0;
            }
            ...
            for (int i = 0; i < u.N_O_SZ; i++) {
                ...
                if (v.N_O_SZ==0) {
                    int hu = nodeLabel2[nodeLabel1[id2].oldID_mapto_newID].h_out;
                    nodeLabel2[nodeLabel1[id].oldID_mapto_newID].L_out[(hu >> 5) % K] |= 1<< (hu & 31);
                } else {
                    for (int j = 0; j < K; j++) {
                        nodeLabel2[nodeLabel1[id].oldID_mapto_newID].L_out[j] |= nodeLabel2[nodeLabel1[id2].oldID_mapto_newID].L_out[j];
                    }
                }
            }
    ...
    void dfs_out(int id,node &u) {
        ...
        if (u.N_O_SZ==0) {
            ...
            for (int i = 0; i < u.N_O_SZ; i++) {
                int t=u.N_O[i];
                node &v = nodes[u.N_O_2[i]];
                if (v.vis != vis_cur) {
                    dfs_out(t,v);
                }
                if (v.N_O_SZ==0) {
                    int hu = nodeLabel2[nodeLabel1[t].oldID_mapto_newID].h_out;
                    nodeLabel2[nodeLabel1[id].oldID_mapto_newID].L_out[(hu >> 5) % K] |= 1 << (hu & 31);
                } else {
                    for (int j = 0; j < K; j++) {
                        nodeLabel2[nodeLabel1[id].oldID_mapto_newID].L_out[j] |= nodeLabel2[nodeLabel1[t].oldID_mapto_newID].L_out[j];
                    }
                }
            }
    ...
    */
    void calculate()
    {
        ...
        {
            node &u=nodes[temp_root_out[i]];
            dfs_out(temp_root_out[i],u);
        }
    }
    ...
    void read_graph(const char *filename) {
        ...
        for (int u = 0; u < n; u++) {
           ...
           // nodes[u].N_O_2 = new int[N_O[u].size()];
           // copy(N_O[u].begin(), N_O[u].end(), nodes[u].N_O_2);
        ...
        for(vector<int>::iterator x=src.begin(), y=dest.begin(); x!=src.end(); x++, y++) {
            ...
            if(*x == *y)
                ...
                if (nodes[*y].N_O_SZ==0) {
                    ...
                    for (int i = 0; i < K; i++) {
                        if ((nodeLabel2[nodeLabel1[*x].oldID_mapto_newID].L_out[i] & nodeLabel2[nodeLabel1[*y].oldID_mapto_newID].L_out[i]) != nodeLabel2[nodeLabel1[*x].oldID_mapto_newID].L_out[i]) {
                            r= false;
                            break;
                        }
                    }
        ...
        for(vector<int>::iterator x=src.begin(), y=dest.begin(); x!=src.end(); x++, y++) {
            if(*x == *y)
                ...
                if (nodes[*y].N_O_SZ==0) {
                    ...
                    for (int i = 0; i < K; i++) {
                        cout<< nodeLabel2[nodeLabel1[*x].oldID_mapto_newID].L_out[i]<<":"<<nodeLabel2[nodeLabel1[*y].oldID_mapto_newID].L_out[i]<<endl;
                        if ((nodeLabel2[nodeLabel1[*x].oldID_mapto_newID].L_out[i] & nodeLabel2[nodeLabel1[*y].oldID_mapto_newID].L_out[i]) != nodeLabel2[nodeLabel1[*x].oldID_mapto_newID].L_out[i]) {
                            r= false;
                            break;
                        }
