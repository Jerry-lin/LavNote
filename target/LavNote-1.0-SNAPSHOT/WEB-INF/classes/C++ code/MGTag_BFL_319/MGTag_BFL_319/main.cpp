...
namespace bs {
    ...
    void dfs_out(int id,node &u) {
        int new_id=nodeLabel1[id].oldID_mapto_newID;
        nodeLabel2[new_id].vis= vis_cur;
        
        if (u.N_O_SZ==0) {
            nodeLabel2[new_id].h_out = h_out() % (K * 32);
        } else {
            for (int i = 0; i < K; i++) {
                nodeLabel2[new_id].L_out[i] = 0;
            }
            
            for (int i = 0; i < u.N_O_SZ; i++) {
                int v_id=u.N_O[i];
                int v_new_id=nodeLabel1[v_id].oldID_mapto_newID;
                node &v = nodes[v_id];
                if (nodeLabel2[v_new_id].vis != vis_cur) {
                    dfs_out(v_id,v);
                }
                if (v.N_O_SZ==0) {
                    int hu = nodeLabel2[v_new_id].h_out;
                    nodeLabel2[v_new_id].L_out[(hu >> 5) % K] |= 1 << (hu & 31);
                } else {
                    for (int j = 0; j < K; j++) {
                        nodeLabel2[new_id].L_out[j] |= nodeLabel2[v_new_id].L_out[j];
                    }
                }
            }
            
            int hu = h_out();
            nodeLabel2[new_id].L_out[(hu >> 5) % K] |= 1 << (hu & 31);
        }
    }
    
    void calculate_out()
    {
        for (int u=0; u<temp_root_out.size(); u++) {
            dfs_out(u,nodes[u]);
        }
    }
    ...
    void index_construction() {
        ...
        totalsize += oldID_mapto_newID_count + temp_root_arr.size()+temp_root_out.size();
    ...
    bool down_layer_to_up_layer_without_cross_located1(int id1, int id2){
        ...
        for(int i = 0; i < len; i ++){
            ...
            //else if(reach_flag[nodeLabel2[id1].id_cross_arr[i]] == querycnt)continue;
                ...
                int cross_id=nodeLabel2[id1].id_cross_arr[i];
                
                //reach_flag[nodeLabel2[id1].id_cross_arr[i]] = querycnt;
                if(nodeLabel2[cross_id].rl < nodeLabel2[id2].rl){
                    for (int j = 0; j < K; j++) {
                        if ((nodeLabel2[cross_id].L_out[j] & nodeLabel2[id2].L_out[j]) != nodeLabel2[id2].L_out[j]) {
                            continue;
                        }
                    }
                    ...
                    if(same_layer_tree_reach1(cross_id, id2)){
                        return true;
                    }
                }
            }
        ...
        for(int i = 0; i < len; i ++){
            ...
            //else if(reach_flag[nodeLabel2[id1].id_cross_arr[i]] == querycnt)continue;
                ...
                if(nodeLabel2[cross_id].rl < nodeLabel2[id2].rl){
                    ...
                    if(same_layer_tree_reach(cross_id, id2)){
                        return true;
                    }
                }
            }
    ...
    void run_queries() {
        ...
        for(vector<int>::iterator x=src.begin(), y=dest.begin(); x!=src.end(); x++, y++) {
            if(*x == *y)
                ...
                if(nodes[*y].N_O_SZ==0)
                {
                    if ((nodeLabel2[nodeLabel1[*x].oldID_mapto_newID].L_out[nodeLabel2[nodeLabel1[*y].oldID_mapto_newID].h_out >> 5] & (1 << (nodeLabel2[nodeLabel1[*y].oldID_mapto_newID].h_out & 31))) == 0) {
                        r=false;
                        continue;
                    }
                }
               ...
               r = query(nodeLabel1[*x].oldID_mapto_newID, nodeLabel1[*y].oldID_mapto_newID);
            }
        }
        gettimeofday(&end_at, 0);
        ...
        gettimeofday(&end_at, 0);
        cout<<"the BFL can judge"<<out_reach<<endl;
