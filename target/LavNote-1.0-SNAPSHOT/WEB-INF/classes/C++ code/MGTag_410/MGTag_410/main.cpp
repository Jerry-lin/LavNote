...
namespace bs {
    ...
    void read_graph(const char *filename) {
        ...
        for (int u = 0; u < n; u++) {
            ...
            sort(N_O_UP[u].begin(), N_O_UP[u].end());
    ...
    void cal_cross_degree_up(bool *&visited_arr, int *&curInDegree){//, unsigned *&nextInDegree){
        int len = temp_root_arr.size();
        deque<int>* que = new deque<int>();
        for(int i = 0; i < len; i ++){
            que->push_front(temp_root_arr[i]);
    ...
    void decode_up(int *&curInDegree){
        ...
        for(int index = 0;index < rootNum; index ++){
            ...
            while(!que->empty()){
                ...
                idcount++;
                nodeLabel3[u].oldID_mapto_newID = idcount;
                nodeLabel4[idcount].rl = nodes[temp_root_arr[index]].root_layer_up;
                nodeLabel4[idcount].rlt = nodes[temp_root_arr[index]].root_located_up;
    ...
    void cal_index_up(){
        ...
        for(int i = TopoOrder.size() - 1; i >= 0; i--){
            ...
            unsigned tmpLup=0, tmpInterval = nodeLabel3[u].oldID_mapto_newID;
            int right = excpID_O[u].size() - 1;
            //ID temp_nor = oldID_mapto_newID[u];
            for(int j=0; j<nodes[u].N_O_SZ_UP; j++){
                int addr = nodes[u].N_O_UP[j];
                int addr_newID = nodeLabel3[addr].oldID_mapto_newID;
                ...
                if(nodes[addr].root_located_up == nodes[u].root_located_up) {
                    //interval
                    if((right < 0 || !is_excp(u, addr, 0, right)) && tmpInterval < nodeLabel4[addr_newID].interval)
                        tmpInterval = nodeLabel4[addr_newID].interval;
                }
                //else //if(cross_located[u][cross_located_count] == 0)
                //id_cross_arr_pro[u].push_back(addr[j]);
            }
            nodeLabel4[nodeLabel3[u].oldID_mapto_newID].interval = tmpInterval;
            //nor_interval[oldID_mapto_newID[u]] = temp_nor;
        }
    }
                ...
                if(nodeLabel2[addr_newID].rlt == nodeLabel2[u_newID].rlt) {
                    ...
                    if(exit_flag[addr_newID] != querycnt + 1 && reach_flag[addr_newID] != querycnt &&
                       !is_reachable(u_newID, addr_newID, len)){
                        id_cross_arr_pro[u_newID].push_back(addr);
    ...
    void cal_excp_and_cross_arr_up(){
        //reach_flag = (int *)calloc((nodes.size() + 1), sizeof(int));
        memset(reach_flag,0,(nodes.size()+1)*sizeof(int));
        int *exit_flag = (int *)calloc((nodes.size() + 1), sizeof(int));
        querycnt = -1;
        //bool flag;
        for(int i = TopoOrder.size() - 1; i >= 0; i--){
            int u = TopoOrder[i];
            int u_newID = nodeLabel3[u].oldID_mapto_newID;
            querycnt += 2;
            int sz = excpID_O[u].size();
            //int cnt = 0;
            for(int n = 0; n < sz; n ++){
                int newID = nodeLabel3[excpID_O[u][n]].oldID_mapto_newID;
                if(nodeLabel4[u_newID].interval < nodeLabel4[newID].interval /*&& exit_flag[newID] != querycnt*/){
                    nodeLabel4[u_newID].excpID.push_back(newID);
                    exit_flag[newID] = querycnt;
                }
            }
            ...
            for(int j=0; j<nodes[u].N_O_SZ_UP; j++){
                ...
                int addr_newID = nodeLabel3[addr].oldID_mapto_newID;
                //int len = id_cross_arr_pro[u_newID].size();
                if(nodeLabel4[addr_newID].rlt == nodeLabel4[u_newID].rlt) {
                    //excpID
                    sz = nodeLabel4[addr_newID].excpID.size();
                    for(int n = 0; n < sz; n ++){
                        if(nodeLabel4[u_newID].interval < nodeLabel4[nodeLabel4[addr_newID].excpID[n]].interval &&
                           exit_flag[nodeLabel4[addr_newID].excpID[n]] != querycnt){
                            nodeLabel4[u_newID].excpID.push_back(nodeLabel4[addr_newID].excpID[n]);
                            exit_flag[nodeLabel4[addr_newID].excpID[n]] = querycnt;
                        }
                    }
                }
            }
            if(nodeLabel4[u_newID].excpID.size() > 0)
                sort(nodeLabel4[u_newID].excpID.begin(), nodeLabel4[u_newID].excpID.end());//, cmpNewID);
        }
        
    }
    
    void index_down_construction()
    {
        ...
        delete[] curInDegree;
        delete[] nextInDegree;
        delete[] cross_vertices_arr;
    }
    
    void index_up_construction()
    {
        temp_root_arr.clear();
        for (int u = 0; u < nodes.size(); u++) {
            if(nodes[u].indegree == 0){
        ...
        nodeLabel3 = (struct label3 *)calloc(nodes.size(), sizeof(struct label3));
        nodeLabel4 = (struct label4 *)calloc(nodes.size() + 1, sizeof(struct label4));
        oldID_mapto_newID_count = 1;
        int *curInDegree =  (int *)calloc(nodes.size(), sizeof(int));
        int *nextInDegree = (int *)calloc(nodes.size(), sizeof(int));
        bool *cross_vertices_arr = (bool *)calloc(nodes.size(), sizeof(bool));
        ...
        memset(cross_vertices_arr, false, sizeof(bool) * nodes.size());
        cal_cross_degree_up(cross_vertices_arr, curInDegree);//, nextInDegree);
    ...
    void index_construction() {
        ...
        index_down_construction();
        index_up_construction();
        ...
        for(int i = 1; i <= nodes.size(); i ++){
            totalsize += nodeLabel2[i].excpID.size() + nodeLabel2[i].id_cross_arr.size()+nodeLabel4[i].excpID.size();
        }
        totalsize += oldID_mapto_newID_count*2+ temp_root_arr.size();
    ...
    bool same_layer_tree_reach_up(int id1, int id2){
        //reach_flag[id1] = querycnt;
        if(nodeLabel4[id1].rlt != nodeLabel4[id2].rlt){
            return false;
        }
        else{
            if(nodeLabel4[id1].interval <= nodeLabel4[id2].interval){
                return true;
            }
            else {
                int len = nodeLabel4[id1].excpID.size();
                for(int i = 0; i < len; i ++) {
                    if(nodeLabel4[id1].excpID[i] > id2)
                        return false;
                    else if(nodeLabel4[nodeLabel2[id1].excpID[i]].interval <= nodeLabel4[id2].interval) {
    ...
    bool queryByOldID(int id1, int id2){
        int id1_new_id=nodeLabel1[id1].oldID_mapto_newID;
        int id2_new_id=nodeLabel1[id2].oldID_mapto_newID;
        if(nodeLabel4[nodeLabel3[id1].oldID_mapto_newID].rl==nodeLabel4[nodeLabel3[id2].oldID_mapto_newID].rl)
             return same_layer_tree_reach_up(nodeLabel3[id1].oldID_mapto_newID,nodeLabel3[id2].oldID_mapto_newID);
        //传入旧id进行比较
        if(nodeLabel2[id1_new_id].rl < nodeLabel2[id2_new_id].rl){
            return down_layer_to_up_layer_without_cross_located(id1, id2);
        }
        else {
            return same_layer_tree_reach(id1_new_id, id2_new_id);
        }
        return false;
    }
