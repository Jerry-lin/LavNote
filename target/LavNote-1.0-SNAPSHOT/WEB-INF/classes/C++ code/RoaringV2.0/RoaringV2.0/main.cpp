...
namespace bs {
    ...
    void read_graph(const char *filename) {
        ...
        for (int u = 0; u < n; u++) {
            ...
            sort(nodes[u].ups.begin(),nodes[u].ups.end());
        ...
        for(int i = TopoOrder.size() - 1; i >= 0; i--){
            ...
            for(int j=0; j<nodes[u].N_O_SZ; j++){
                ...
                if(nodeLabel2[addr_newID].rlt == nodeLabel2[u_newID].rlt) {
                    ...
                    for(Roaring::const_iterator i = nodeLabel2[addr_newID].bits.begin() ; i != nodeLabel2[addr_newID].bits.end() ; i++){
                        if(exit_flag[*i] == querycnt + 1 ||
                           reach_flag[*i] == querycnt)
                            continue;
                        
                        if(!is_reachable(u_newID, *i, len)){
                            
                            id_cross_arr_pro[u_newID].push_back(*i);
                            exit_flag[*i] = querycnt + 1;
                        }
                    }
                }
                else {
            ...
            for(int j = 0; j < sz; j ++){
                ...
                {
                    nodeLabel2[u_newID].bits.add(id_cross_arr_pro[u_newID][j]);
    ...
    void index_construction() {
        ...
        for (int i=0; i<nodes.size(); i++) {
            for (int j=0; j<nodes[i].ups.size(); j++) {
                nodeLabel2[i].ups.push_back(nodes[i].ups[j]);
            }
        }
        ...
        for(int i = 1; i <= nodes.size(); i ++){
            totalsize += nodeLabel2[i].excpID.size()+nodeLabel2[i].ups.size();
            bytes=nodeLabel2[i].bits.getSizeInBytes();
        }
        totalsize += oldID_mapto_newID_count + temp_root_arr.size();
        
        cout<<"index size (MB): "<<(double)(totalsize*4+bytes)/(1024*1024)<<endl;
        cout<<"index size (KB): "<<(double)(totalsize*4+bytes)/1024<<endl;
    }
    ...
    bool down_layer_to_up_layer_without_cross_located1(int id1, int id2){
        ...
        for(Roaring::const_iterator i = nodeLabel2[id1].bits.begin() ; i != nodeLabel2[id1].bits.end() ; i++){
            if(*i > id2)
                return false;
            ...
            //else if(reach_flag[nodeLabel2[id1].id_cross_arr[i]] == querycnt)continue;
                ...
                if(nodeLabel2[*i].rl < nodeLabel2[id2].rl){
                    for (int j = 0; j < K; j++) {
                        if ((nodeLabel2[*i].L_out[j] & nodeLabel2[id2].L_out[j]) != nodeLabel2[id2].L_out[j]) {
                            continue;
                        }
                    }
                }
                ...
                if(nodeLabel2[*i].rl < nodeLabel2[id2].rl){
                    //traverseCnt ++;
                    if(down_layer_to_up_layer_without_cross_located1(*i, id2))
                        return true;
                    
                }
                else {
                    if(same_layer_tree_reach1(*i, id2)){
                        return true;
                    }
                }
            }
...
bool down_layer_to_up_layer_without_cross_located(int id1, int id2){
        ...
        for(Roaring::const_iterator i = nodeLabel2[id1].bits.begin() ; i != nodeLabel2[id1].bits.end() ; i++){
            if(*i > id2)
                ...
                if(nodeLabel2[*i].rl < nodeLabel2[id2].rl){
                    //traverseCnt ++;
                    if(down_layer_to_up_layer_without_cross_located(*i, id2))
                        return true;
                    
                }
                else {
                    if(same_layer_tree_reach(*i, id2)){
                        return true;
                    }
                }
            }
    ...
    void run_queries() {
        ...
        for(vector<int>::iterator x=src.begin(), y=dest.begin(); x!=src.end(); x++, y++) {
            ...
            if(*x == *y)
                ...
                for (int i = 0; i < K; i++) {
                    if ((nodeLabel2[nodeLabel1[*x].oldID_mapto_newID].L_out[i] & nodeLabel2[nodeLabel1[*y].oldID_mapto_newID].L_out[i]) != nodeLabel2[nodeLabel1[*y].oldID_mapto_newID].L_out[i]) {
                        r=false;
                        break;
                    }
                }
                if(!r) continue;
                for (int i = 0; i < nodeLabel2[*x].ups.size(); i++) {
                    int id=nodeLabel2[*x].ups[i];
                   
                    for (int i = 0; i < K; i++) {
                        if ((nodeLabel2[id].L_out[i] & nodeLabel2[id].L_out[i]) != nodeLabel2[id].L_out[i]) {
                            r=false;
                            break;
                        }
